...
void tlb_add(int index, page_node entry) {
    for (index; index<TLB_SIZE; ++index){
        if (entry.is_present == false)
            tlb[index].frame_num = tlb[index-1].frame_num;
            tlb[index].npage = tlb[index-1].npage;
    }
 }  //// TODO
...
void tlb_hit(size_t& frame, size_t& page, size_t& tlb_hits, int result) {
    for (int i = 0; i < TLB_SIZE; i++) {
        if (tlb[i].npage == page && tlb[i].frame_num == frame) 
            {tlb_hits++;}
        result=tlb_hits;
    }
    for (int i = 0; i < TLB_SIZE; i++) {
    ...
    //for (int i = 0; i < TLB_SIZE; i++){
    //    if (tlb[i].npage == page && tlb[i].npage == frame){result = tlb_hits;}
    //}      
  }  /////// TODO
...
void tlb_miss(size_t& frame, size_t& page, size_t& tlb_track) { 
    ...
    for (int i = 0; i < TLB_SIZE; i++){
        tlb[tlb_track].npage = tlb[tlb_track-1].npage;
        tlb[tlb_track].frame_num = tlb[tlb_track-1].frame_num;
...
void fifo_replace_page(size_t& frame ) {
    ...
    //for (int i = 0; i < TLB_SIZE; i++){
    //    if (tlb[i].npage == frame){
    //        //tlb[frame].npage = page;
    //        tlb[frame].frame_num = frame;
    //    }
    //    else{
    //        tlb[frame-1].frame_num = frame;
    //    }        
    //}
 }   // TODO
